...
model = dict(
    ...
    panoview_reader=dict(
        ...
        ds_num_filters=[32, 64, 128, 128],
        us_layer_strides=[1, 2, 4, 8],
        fpn_num_filters=32,
        us_num_filters=[32, 32, 32, 32],
...
train_preprocessor = dict(
    ...
    time_stamp_as_last_feature=False, # use ring instead, good for single training
)
...
panoview_projector = dict(
    lidar_xyz=[0, 0, 0], 
    h_steps=(-180, 180, 0.375), 
    v_steps=(-30, 10, 1.25),
)

train_pipeline = [
    dict(type="LoadPointCloudFromFile", dataset=dataset_type, num_point_feature=5),
    dict(type="LoadPointCloudAnnotations", with_bbox=True),
    dict(type="Preprocess", cfg=train_preprocessor),
    dict(type="PanoviewProjection", cfg=panoview_projector),
